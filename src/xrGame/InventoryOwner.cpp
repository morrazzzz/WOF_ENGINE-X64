...
CActorBackpack* CInventoryOwner::GetBackpack() const
{
	return dynamic_cast<CActorBackpack*>(inventory().ItemFromSlot(BACKPACK_SLOT));;
}
...
float  CInventoryOwner::MaxCarryWeight () const
{
	...
	const CActorBackpack* backpack = GetBackpack();
	if (backpack)
		ret += outfit->m_additional_weight2;
