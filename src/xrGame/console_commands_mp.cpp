...
public:
	...
	virtual void Execute(LPCSTR args)
	{
		if (OnServer())
		{
			string256 ban_user = { 0 };
			exclude_raid_from_args(args, ban_user, sizeof(ban_user));

#ifndef MP_SAVE_JSON
			string_path filepath;
			FS.update_path(filepath, "$mp_saves_logins$", "logins.ltx");
			CInifile* file = xr_new<CInifile>(filepath, false, true);
			if (file && file->section_exist(ban_name))
				file->w_bool(ban_name, "banned", true);
			file->save_as(filepath);
#else 
			...
			if (jsonMAIN.has<Array>("ACCOUNTS:"))
			{
				...
				{
					if (jsonArr.get<Object>(i).has<String>("login"))
					{
						LPCSTR login_name = jsonArr.get<Object>(i).get<String>("login").c_str();
 						if (xr_strcmp(login_name, ban_user) == 0)
						{
							Msg("Account Banned");
							jsonArr.get<Object>(i) << "ban_user" << Boolean(true);
						}
					}
			...
			IWriter* file_W = FS.w_open(path_xray);
			if (file_W)
			{
				file_W->w_string(jsonMAIN.json().c_str());
				FS.w_close(file_W);
		...
		else
		{
			...
			P.w_begin(M_REMOTE_CONTROL_CMD);
			string128 str;
			xr_sprintf(str, "adm_ban_account %s", args);
			P.w_stringZ(str);
			Level().Send(P, net_flags(TRUE, TRUE));
		}
	}
...
public:
	...
	virtual void Execute(LPCSTR args)
	{
		if (OnServer())
		{
			string256 unban_user = {0};
			exclude_raid_from_args(args, unban_user, sizeof(unban_user));
			...
			if (jsonMAIN.has<Array>("ACCOUNTS:"))
			{
				...
				for (int i = 0; i != jsonArr.size(); i++)
				{
					...
					{
						LPCSTR login_name = jsonArr.get<Object>(i).get<String>("login").c_str();
						if (strstr(login_name, unban_user) == 0)
							jsonArr.get<Object>(i) << "ban_user" << Boolean(false);
					}
				}
			}
		...
		else
		{
			...
			P.w_begin(M_REMOTE_CONTROL_CMD);
			string128 str;
			xr_sprintf(str, "adm_unban_account %s", args);
			P.w_stringZ(str);
			Level().Send(P, net_flags(TRUE, TRUE));
		}
	}
...
class CCC_WriteItemsToLog : public IConsole_Command
{
public:
	CCC_WriteItemsToLog(LPCSTR N) : IConsole_Command(N) { bEmptyArgsHandled = true; };
