...
void game_sv_freemp::Update()
{
	...
	if (Level().game && Device.dwFrame % 120 == 0)
	{
...
void game_sv_freemp::CraftItem(NET_Packet P, ClientID sender)
{
	...
	if (ps)
	{
		...
		if (actor)
		{
			...
			TIItemContainer items_actor;
			actor->inventory().AddAvailableItems(items_actor, false);
	...
	if (!crafted)
	{
		...
		Packet.w_stringZ("Сервер Невыдал Крафт");
	...
	else
	{
		...
		Packet.w_begin(M_GAMEMESSAGE);
		Packet.w_u32(GE_CRAFT_ITEM);
		Packet.w_stringZ("Сервер Выдал Крафт");
		server().SendTo(sender, Packet, net_flags(true, true));
	}
